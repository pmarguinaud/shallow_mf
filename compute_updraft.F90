...
DO JSV=1,ISV
  ...
  CALL MZM_MF(KLON,KIDIA,KFDIA,KLEV,KKA,KKU,KKL,PSVM(:,:,JSV), ZSVM_F(:,:,JSV))
END DO
!                     
...
IF (OENTR_DETR) THEN
  ...
  PRSAT_UP(KIDIA:KFDIA,KKB) = ZRSATW(KIDIA:KFDIA)*(1-PFRAC_ICE_UP(KIDIA:KFDIA,KKB)) + ZRSATI(KIDIA:KFDIA)*PFRAC_ICE_UP(KIDIA:KFDIA,KKB)
  ...
  WHERE (ZWTHVSURF(KIDIA:KFDIA)>0.)
    PEMF(KIDIA:KFDIA,KKB) = XCMF * ZRHO_F(KIDIA:KFDIA,KKB) * ((ZG_O_THVREF(KIDIA:KFDIA,KKB))*ZWTHVSURF(KIDIA:KFDIA)*ZLUP(KIDIA:KFDIA))**(1./3.)
...
! Compute non cons. var. at level JK+KKL
  ...
  WHERE(GTEST(KIDIA:KFDIA))
    ...
    PRSAT_UP(KIDIA:KFDIA,JK+KKL) = ZRSATW(KIDIA:KFDIA)*(1-PFRAC_ICE_UP(KIDIA:KFDIA,JK+KKL)) + ZRSATI(KIDIA:KFDIA)*PFRAC_ICE_UP(KIDIA:KFDIA,JK+KKL)
  ENDWHERE
